...
Vector3f AC_CustomControl_PID::update()
{
    ...
    attitude_target = _atti_control->get_attitude_target_quat();
    // This vector represents the angular error to rotate the thrust vector using x and y and heading using z
    Vector3f attitude_error;
    float _thrust_angle, _thrust_error_angle;
    _atti_control->thrust_heading_rotation_angles(attitude_target, attitude_body, attitude_error, _thrust_angle, _thrust_error_angle);
    ...
    Vector3f ang_vel_body_feedforward = rotation_target_to_body * _atti_control->get_attitude_target_ang_vel();
...
// Note that integrator are not reset correctly as it is done in reset_main_atti_controller inside AC_CustomControl.cpp
